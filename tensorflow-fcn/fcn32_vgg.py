...
class FCN32VGG:
    ...
    def __init__(self, vgg16_npy_path=None):
        ...
        if not os.path.isfile(vgg16_npy_path):
            ...
            sys.exit(1)
        #data_dict['name'][0] contains the weights and data_dict['name'][1] contains the bias
    ...
    def _fc_layer(self, bottom, name, num_classes=None,
        ...
        with tf.variable_scope(name) as scope:
            ...
            #reshape the weights of fully connected layers into filter 
            if name == 'fc6':
                filt = self.get_fc_weight_reshape(name, [7, 7, 512, 4096])
            elif name == 'score_fr':
                name = 'fc8'  # Name of score_fr layer in VGG Model
                #check if we need to reduce the numclasses 
    ...
    def _upscore_layer(self, bottom, shape,
        ...
        with tf.variable_scope(name):
            ...
            print('new_shape is ')
            print(tf.shape(output_shape))

            logging.debug("Layer: %s, Fan-in: %d" % (name, in_features))
            f_shape = [ksize, ksize, num_classes, in_features]
            print('filter shape is ....')
            print(f_shape)
    ...
    def get_deconv_filter(self, f_shape):
        ...
        print('height is %s '%heigh)
        f = ceil(width/2.0)
        c = (2 * f - 1 - f % 2) / (2.0 * f)
        bilinear = np.zeros([f_shape[0], f_shape[1]])
        print('the shape of bilinear is ')
    ...
    def get_conv_filter(self, name):
        ...
        #creat a new variable named filter 
    ...
    def get_fc_weight_reshape(self, name, shape, num_classes=None):
        ...
        #Only the final layer has parameterf num_classes , check if we need to reduce the num_classes to desired new_num_classs
        #by taking the average of n adjacent class
...
with tf.Session() as sess:
    ...
    scp.misc.imsave('fcn32_downsampled_2.png', down_color)
    scp.misc.imsave('fcn32_upsampled_2.png', up_color)
