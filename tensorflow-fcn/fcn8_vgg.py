...
import read_Pascal_VOC as scene_parsing
import datetime
import BatchDatsetReader as dataset
import test_batch_reader as test_dataset
import numpy as np
...
tf.flags.DEFINE_string("data_dir", "VOCdevkit/VOC2012/", "path to dataset")
...
class FCN8VGG:
    ...
    def build(self, rgb, train=False, num_classes=21, random_init_fc8=False,
        ...
        self.fc7 = self._fc_layer(self.fc6, "fc7",debug = debug)
        if train:
    ...
    def main(self):
        ...
        pred_annotation, logits = self.build(image,train = True,random_init_fc8 = True,debug = True , num_classes = 21)
        ...
        if FLAGS.mode =='test':
            test_dataset_reader = test_dataset.BatchDatset(test_records,image_options)
        sess = tf.Session()
        ...
        if FLAGS.mode == "train":
            ...
            print(pred.shape)
            pred = np.squeeze(pred,axis=3)
            utils.save_image(pred[0].astype(np.uint8),FLAGS.logs_dir,name = "test_")
